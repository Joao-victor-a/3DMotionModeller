...
class Ball(SimulatedBody):
    ...
    def __init__(self, mass, radius, step_time=0.01):
        In = 2 / 5 * mass * radius ** 2
        SimulatedBody.__init__(self, mass, [0, 0, 0], [[In, 0, 0], [
                               0, In, 0], [0, 0, In]], step_time=step_time)
    ...
    def update_forces(self, t, p, v, a, w, alpha, theta):
        self.air_drag.vect = -v * \
            np.linalg.norm(v) * 0.5 * self.cd * rho_air * self.area
...
class ProjectileBall(SimulatedBody):
    ...
    def __init__(self, mass, radius, step_time=0.01):
        In = 2 / 5 * mass * radius ** 2
        SimulatedBody.__init__(self, mass, [0, 0, 0], [[In, 0, 0], [0, In, 0], [0, 0, In]],
...
class BlockOnSpringForced(SimulatedBody):
    ...
    def __init__(self, mass, c, k, width=1, initial_displacement=0, step_time=0.01):
        ...
        self.external_force = TimeVaryingForce(
            lambda t: np.array([np.sin(10 * t), 0, 0]))
...
class Moon(SimulatedBody):
    ...
    def __init__(self, v_initial, step_time=0.01):
        ...
        self.dist = 384400000
        In = 2 / 5 * mass * radius**2
        SimulatedBody.__init__(self, mass, [0, 0, 0], [[In, 0, 0], [0, In, 0], [
                               0, 0, In]], step_time=step_time, p_initial=[self.dist, 0, 0], v_initial=[0, v_initial, 0])
